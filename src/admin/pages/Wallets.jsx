...
export default function Wallets() {
  const [systemWallet, setSystemWallet] = useState(890500);
  const [totalRevenue, setTotalRevenue] = useState(124500);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [withdrawAmount, setWithdrawAmount] = useState('');
  const [history, setHistory] = useState([
    { id: "WD104", amount: "₹5,000.00", date: "2026-08-01", method: "Razorpay", status: "Completed" },
    { id: "WD103", amount: "₹10,000.00", date: "2026-07-15", method: "Razorpay", status: "Completed" },
  ]);
  ...
  const columns = [
    { header: "Withdrawal ID", accessor: "id" },
    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-admin-surface-container border border-admin-outline-variant rounded-xl shadow-lg overflow-hidden flex flex-col">
        <div className="p-6 border-b border-admin-outline-variant flex items-center justify-between">
          <h3 className="text-lg font-semibold text-admin-on-surface">Withdrawal History</h3>
        </div>
